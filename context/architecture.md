...
- Playwright reporting now also includes a repo-local helper under `scripts/test/` that reads the final Playwright JUnit artifact, normalizes suite/test/step metadata plus optional branch/run metadata into a report model, and writes a self-contained standalone HTML companion artifact to `test-results/playwright/report.html`.

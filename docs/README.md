...
Need the short version first? See [QUICKSTART.md](QUICKSTART.md).
...
- [Historical Context](../context/README.md)

## Project Overview

This project is a browser-based Tic Tac Toe app built as a small lab for modern frontend, game-domain, testing, and deployment workflows. It includes local single-player play against a deterministic CPU opponent plus multiplayer create, join, spectate, replay, and live-update flows backed by a TypeScript serverless backend.

The repository is structured so the game rules stay framework-agnostic, the UI remains thin, and the deployed stack is managed through Terraform-backed AWS infrastructure.
...
Run the combined frontend unit plus end-to-end workflow:
...
- `src/multiplayer`
  Frontend API clients, multiplayer types, name generation, and local participant-session storage.
- `src/components`
  Reusable presentational UI pieces for the board, status area, action bar, and overlays.
- `src/routes`
  Route-level page composition for the landing page plus CPU and multiplayer game detail flows.
- `backend/src`
  Multiplayer domain logic, HTTP handlers, WebSocket handlers, and DynamoDB repository code for the AWS backend.
- `tests`

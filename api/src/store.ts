...
import { MAX_CONCURRENT_GAMES } from "./domain.js";
...
const isConcurrentState = (state: StoredGame["state"]): boolean =>
...
export const saveGameMutation = async (
  ...
  const shouldDecrementCounter =
    isConcurrentState(previousGame.state) && !isConcurrentState(nextGame.state);
